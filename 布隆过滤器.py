"""
	布隆过滤器的data用一个数组存储，目的是可以存储多个扩容后的二进制表
	size表示当前二进制表的尺寸大小
	num表示当前表中已经改动的二进制个数
	（无论有没有重复，每添加一个数据num就增加3，目的是最大限度减少冲突）
	stowage是装载因子，最大限度为0.8，大于0.8则增加一个大小为原来一倍的新表到data中
"""
class Bloom_Filter():
	def __init__(self, number):
		self.data = [bytearray(number)]
		self.size = number
		self.num = 0
		self.stowage = self.num / self.size

	# ASCll码法
	def hash_function_1(self, data, size):
		if type(data) != type("1"):
			data = str(data)
	...
	def hash_function_2(self, data, size):
		if type(data) != type("1"):
			data = str(data)
	...
	# 平方求余
	def hash_function_3(self,data,size):
		if type(data) != type(1):
			num = 0
			sumer = 0
			for i in data:
