class Node():
	def __init__(self,element,x_way,y_way):	
		self.data=element
		self.value=[]	#value用来存储该节点与指向节点之间的权值
		self.x=x_way	#x，y属性存储该节点的x，y值
		self.y=y_way

class Heap():					#实现一个小顶堆
	def __init__(self):			#小顶堆用于后续的A*算法的优先级队列
		self.data=[None]

	def add_data(self,element):				#小顶堆存储的是一个元组，元组有两个值，第一个是节点在邻接表中对应的下标
		self.data.append(element)			#第二个是该节点在A*算法中judge数组中的值
...
class Adjacency_list():			#邻接表
	...
	def add_data(self,start,start_x,start_y,end,end_x,end_y,weight):
		...
		if start not in self.mapping:
			...
			start_num=self.mapping.index(start)			#如果起始节点已经存在，则不更新起始节点的坐标值，结束节点同理
	...
	def A(self,start,end):
		def manhattan(start_num,end_num):				#计算曼哈顿距离
			start_x,start_y=self.node_mapping[start_num].x,self.node_mapping[start_num].y
		...
		if start not in self.mapping or end not in self.mapping:
			...
			vertexes=[None]*len(self.mapping)				#vertexes数组用来存储某个下标节点与起始节点的距离，记作g(i)，None表示无穷大
			judge=[None]*len(self.mapping)					
			#judge数组用来存储某个下标节点与结束节点的曼哈顿距离（记作h(i)）与对应下标的vertexes数组中的值之和，记作f(i)，None表示无穷大
			...
			while len(level_queue.data)>1 and find==False:
				minvertex=level_queue.get_top_element()		#取出一个judge值最短的节点
