class Node():										#AC自动机建立在Trie树之上
	def __init__(self,string,pointer=None):			#Trie树中存储的字符串种类只包含26个英文小写字母
		self.data=string							#length记录该字符在字符串中的下标
		self.length=0								#end表示该字符是否为一个模式串的结尾
		self.next=[None]*26
		self.end=False
		self.fail=pointer
