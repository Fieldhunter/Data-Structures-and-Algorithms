...
def bubble_sort(data_list): 					#冒泡排序
	num=len(data_list)
...
def insertion_sort(data_list): 					#插入排序
	num=len(data_list)
	for i in range(num):
		m=i
		while m>0:
...
def selection_sort(data_list):	 				#选择排序
	num=len(data_list)
	for i in range(num):
		min_pos=i
...
def bucket_sort(data_list): 					#桶排序
	list_1=[]									#设置对0-1000范围的数进行排序
	list_2=[]
	list_3=[]
	for i in data_list:
		if i>=0 and i<=333:
...
def radix_sort(data_list,pos=0): 					#基数排序
	if len(data_list)==1:							#设置为排序电话号码
		return data_list
	else:
		now_pos=pos
		num=len(data_list)
