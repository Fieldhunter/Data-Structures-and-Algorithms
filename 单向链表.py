class Node():
	def __init__(self,data):		#存储的数据为数字
		self.data=data
		self.next=None
...
class Linkedlist():
	...
	def add_data(self,num,pos_num=None):		#num是要加入的数字，pos_num代表在哪个数据后面加入
		new_node=Node(num)						#不填默认在头结点
...
def merge(list1,list2,list3): 					#两个链表合并
	data1=list1.head
	data2=list2.head
