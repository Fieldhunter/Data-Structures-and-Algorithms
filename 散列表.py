class Node():							#该散列表存储的是字符串格式数据，装载因子设置为0.2~0.8，利用链表法解决散列冲突
	def __init__(self,num):				#散列函数为计算字符串上每个位置的Unicode码之和，之后求平均值并和散列表大小取余
		self.data=num
		self.next=None
...
class Hash_table():
	def __init__(self,size=10):			#size是散列表的大小，默认为10，num是已有的数据个数
		self.min_size=size				#min_size不扩容时不更新，用于动态缩容时散列表最小大小
		self.size=size					#stowage是装载因子,expansion代表散列表是否正在扩容,new_table存储新散列表
		self.num=0						#扩容策略为当要扩容时，单单申请，不转移数据，当要增加一个数据时，
		self.expansion=False			#将新数据和原散列表中的几个数据加入到新散列表
		self.expansion_pos=0			#expansion_pos代表扩容时原散列表遍历到的位置，目的是方便删除和查找操作
		self.new_table=None
	...
	def old_data_move(self):
		for _ in range(3):					#将老数据加入到新散列表,一次性最多搬3个
			while self.expansion_pos!=len(self.data) and self.data[self.expansion_pos]==None:
				self.expansion_pos+=1
			if self.expansion_pos==len(self.data):		#判断是否原散列表数据已经搬移完，如果已经搬移完了，就把新的散列表更新到原散列表
				...
				self.data[self.expansion_pos].head=self.data[self.expansion_pos].head.next			#对原散列表链表处理
	...
	def add_data(self,data):
		...
		if self.expansion==False:
			...
			if self.stowage>0.8:			#动态扩容
				self.expansion=True
				new_hash_table=Hash_table(self.size*2)
				self.new_table=new_hash_table
				self.old_data_move()
		else:													#扩容中
			hash_value=self.new_table.hash_function(data)		#先将新数据加入
	...
	def del_data(self,data):
		...
		hash_value=self.hash_function(data)
		find=False
		if self.data[hash_value]==None:					#先在原散列表中找
			pass
		else:
		...
		if find:
			...
			self.num-=1
			self.stowage=self.num/self.size
		elif self.expansion==True:					#判断是否正在扩容
		...
		if find and self.expansion==True:				#如果删除成功并处于扩容状态，那么进行数据搬移操作
			self.old_data_move()

		if self.stowage<0.2 and self.size>self.min_size and self.expansion==False:			#动态缩容
